...
// CopyrightFooter — the "© {year} Learning Web, Inc." line for app surfaces
...
const TERMS_URL = getPlatformUrl('/terms') || 'https://www.example.com/terms'
const PRIVACY_URL = getPlatformUrl('/privacy') || 'https://www.example.com/privacy'
  ...
  return (
    <div className={`pb-8 pt-6 text-center px-6 ${className}`}>
      <p className="text-[13px] text-black/30 font-medium">
        {t('auth.terms_text', { defaultValue: "By continuing, you agree to Learning Web's" })}{' '}
        <Link
          to={TERMS_URL}
          target="_blank"
  ...
  return (
    <footer className={`w-full py-6 px-6 ${className}`}>
      <div className="flex flex-col sm:flex-row items-center justify-center gap-x-5 gap-y-2 text-[13px] font-medium">
        <p className={base}>
          {t('common.copyright', { defaultValue: '© {{year}} Learning Web, Inc.', year })}
        </p>
