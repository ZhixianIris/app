...
interface BlockQuizQuestion {
  ...
  // 'single' = pick one, 'multiple' = select all that apply. Generated by the
  // API; absent on older payloads, so read it through resolveQuizResponseType.
...
function questionResponseType(question: BlockQuizQuestion): QuizResponseType {
  return resolveQuizResponseType(
  ...
  const toggleCorrect = (qid: string, aid: string) => {
    ...
    setQuiz({
      ...
      questions: quiz.questions.map((q) => {
        ...
        const wasCorrect = !!q.answers.find((a) => a.answer_id === aid)?.correct
        // Keep the key consistent with the question's response mode: on a
        // pick-one question, marking an answer correct un-marks the previous one.
        const isSingle = questionResponseType(q) === QUIZ_RESPONSE_SINGLE
        return {
          ...q,
          answers: q.answers.map((a) => {
            if (a.answer_id === aid) return { ...a, correct: !wasCorrect }
            if (isSingle && !wasCorrect) return { ...a, correct: false }
            return a
          }),
        }
      }),
                  ...
                  <div key={q.question_id} className="bg-white rounded-lg p-4 nice-shadow">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="px-2 py-0.5 rounded-md bg-neutral-100 text-neutral-500 text-[11px] font-bold">
                        {questionResponseType(q) === QUIZ_RESPONSE_SINGLE
                          ? 'Single answer'
