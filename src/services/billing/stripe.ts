...
export async function createPackCheckoutSession(
  ...
  const APP_URL = appUrl();
  // Return to the org's billing page (which already carries `?org=`), or the
  // hub org list when there's no org context. `sep` picks ?/& accordingly.
  const returnBase = orgSlug ? `${APP_URL}/billing?org=${orgSlug}` : `${APP_URL}/organizations`;
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${returnBase}${sep}pack_purchased=${packId}&session_id={CHECKOUT_SESSION_ID}`,
    cancel_url:  `${returnBase}${sep}checkout=cancelled&pack=${packId}`,
  });
